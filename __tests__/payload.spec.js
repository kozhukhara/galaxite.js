...
describe('Request with payload', () => {
    ...
    it('Parses JSON payload', async () => {
        const testBody = { fields: { textField1: Date.now(), textField2: Date.now().toString(32) }, files: {} }
    ...
    it('Parses `x-www-form-urlencoded` payload', async () => {
        const testBody = { fields: { textField1: Date.now().toString(), textField2: Date.now().toString(32) }, files: {} }
        let formattedParams = new URLSearchParams(testBody.fields);
        const response = await fetch(`http://localhost:${port}/post`, {
            method: 'POST',
    ...
    it('Parses `multipart/form-data` payload', async () => {
        const testBody = { fields: { textField: Date.now().toString() }, files: {} }
        ...
        expect(JSON.stringify(data.fields)).toEqual(JSON.stringify(testBody.fields));
